...
// Manual verification (two-tab SOUL isolation):
// 1) Open http://localhost:5173/#/hermes/chat
// 2) Create session A, pick SOUL = Ryn in the header inline picker
// 3) Open a second browser tab to the same URL
// 4) Create session B in tab 2, pick SOUL = Zen in the header inline picker
// 5) Send messages in both tabs — each shows its own SOUL badge,
//    and each POST /v1/runs carries the correct soul_id for that session
