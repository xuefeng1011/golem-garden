...
- [x] O-5: Fable 기준점 기록 — 2026-06-10 v1 5/5 (fable/sonnet/haiku 전부) + **2026-06-12 v2 포함 풀 스위트: claude-fable-5 7/7** (verdict-format 1회 일시 스폰오류 tokens_out=0/721ms → 재시도 pass — eval 러너에 "tokens_out=0 자동 재시도" 개선 후보). **v2 변별력 결과: sonnet·haiku도 v2 hard 2종 통과** — pass율 천장 효과는 v2에서도 유지. 해석: 이 엔진의 태스크 프로파일에서는 하네스(계약·가드)가 받쳐주면 하위 모델이 Fable과 동급 — 모델 비종속 하네스 논지의 라이브 실증. 스위트의 회귀 감지 역할은 유효하며, pass율 변별이 필요하면 v3(멀티파일·장호라이즌)는 선택 과제
