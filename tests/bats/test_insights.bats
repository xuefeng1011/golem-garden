...
EOF
  ...
  # 픽스처 growth log: success 2 + fail 1 + timeout 1 + turn_cap 2
...
{"date":"2026-07-12","soul":"zzfixture","task":"t7","result":"partial","model":"haiku","cost_usd":0.01}
...
@test "insights: 실패 유형 분해 — result 값별 정확 집계 + 미지 값은 기타" {
  ...
  [ "$status" -eq 0 ]
  # 총계: result:"..." 라인 7건만 (잡라인 배제)
  [[ "$output" == *"태스크: 7건 (성공 2"* ]]
  [[ "$output" == *"실패(fail): 1건"* ]]
  [[ "$output" == *"타임아웃: 1건"* ]]
  [[ "$output" == *"턴캡: 2건"* ]]
  # 미지 값(partial)은 사라지지 않고 기타로 노출
  [[ "$output" == *"기타: 1건"* ]]
}
